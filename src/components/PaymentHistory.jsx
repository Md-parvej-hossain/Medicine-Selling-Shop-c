...
const PaymentHistory = () => {
  const { user } = useAuth();
  const [cart] = useCart();
  const invoiceRef = useRef(null); // Initialize useRef with null
  console.log(cart);

  const totalPrice = cart.reduce((total, item) => total + item.price, 0);
  const handlePrint = useReactToPrint({
    content: () => invoiceRef.current, // This should return the ref
    documentTitle: 'Invoice', // Optional: Set the document title
    onBeforeGetContent: () => {
